...
import com.lmd.thomas.gamechoice.utils.DeviceUtils;
import com.lmd.thomas.gamechoice.wordSearchGenerator.models.Node;
...
public class WordSearchGridAdapter extends BaseAdapter {
    ...
    public View getView(final int pos, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            int size;
            int difference = ((WordSearchManager.ADVANCED_MAX_WORDLENGTH + WordSearchManager.ADVANCED_MAX_DIMENSION_OFFSET - WordSearchManager.MIN_WORDLENGTH) / 6);
            if (WordSearchManager.MIN_WORDLENGTH <= mWordSearchDimension && mWordSearchDimension < (WordSearchManager.MIN_WORDLENGTH + difference))
                size = 36;
            else if ((WordSearchManager.MIN_WORDLENGTH + difference) <= mWordSearchDimension && mWordSearchDimension < (WordSearchManager.MIN_WORDLENGTH + difference * 2))
                size = 34;
            else if ((WordSearchManager.MIN_WORDLENGTH + difference * 2) <= mWordSearchDimension && mWordSearchDimension < (WordSearchManager.MIN_WORDLENGTH + difference * 3))
                size = 30;
            else if ((WordSearchManager.MIN_WORDLENGTH + difference * 3) <= mWordSearchDimension && mWordSearchDimension < (WordSearchManager.MIN_WORDLENGTH + difference * 4))
                size = 28;
            else if ((WordSearchManager.MIN_WORDLENGTH + difference * 4) <= mWordSearchDimension && mWordSearchDimension < (WordSearchManager.MIN_WORDLENGTH + difference * 5))
                size = 24;
